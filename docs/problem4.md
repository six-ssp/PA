...
当前模型额外假设固体骨架沿径向均匀比例收缩，即材料速度为 `v_r=r R'(t)/R(t)`。在此假设下，`x` 才是随固体骨架收缩的材料坐标，固定 `x` 对应同一材料位置。将原方程的时间变化解释为材料导数后，方程中不再额外加入收缩对流项；几何收缩通过 `1/R(t)^2` 的扩散、导热尺度及表面 Robin 边界中的 `R(t)` 体现。

这不是仅作 `x=r/R(t)` 换元就能自动得到的结论。若原方程采用固定空间位置的时间偏导，换元后会出现 `x R'(t)/R(t) ∂u/∂x` 项。附件 2 的表面半径只能提供外边界运动，不能单独验证内部速度假设。本文采用材料坐标近似，不将其宣称为已验证的完整收缩连续介质模型。
