...
def main():
    ...
    record={'source_sha256':before,'check_code_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
            'rows':rows,'purpose':'Diagnose nonmonotonicity in dry-boundary coarse-grid scan; not independent experimental validation.'}
    (DATA/'low_moisture_grid.json').write_text(json.dumps(record,ensure_ascii=False,indent=2),encoding='utf-8')
